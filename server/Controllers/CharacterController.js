...
export const createCharacter = async (req, res) => {
    try {
        ...
        const newCharacter = new Character({
            ...
            inventory: {
                money: {
                    cp: toNum(d.cp), sp: toNum(d.sp), ep: toNum(d.ep),
                    gp: toNum(d.gp), pp: toNum(d.pp),
                },
        ...
        res.status(201).json({ success: true, characterId: saved._id });
...
export const getCharacters = async (req, res) => {
    try {
        ...
        const characters = await Character.find({ userId: req.user.id }).lean();
        return res.status(200).json({ success: true, characters });

    } catch (error) {
        console.error("GET CHARACTERS ERROR:", error);
        res.status(500).json({ success: false, message: "Chyba pri načítaní postáv", error: error.message });
    }
};
...
export const getCharacter = async (req, res) => {
    try {
        ...
        if (!character) {
            return res.status(404).json({ success: false, message: "Postava nenájdená" });
        }

        // Prevent users from accessing other users' characters
        if (character.userId.toString() !== req.user.id) {
            return res.status(403).json({ success: false, message: "Prístup zamietnutý" });
        }
        ...
        res.status(500).json({ success: false, message: "Chyba pri načítaní postavy", error: error.message });
    }
};

// ─── UPDATE character by ID ───────────────────────────────────────────────────
// Accepts the full character object from CharacterSheet.jsx's handleSave()
export const updateCharacter = async (req, res) => {
    try {
        ...
        // Merge the incoming data with the existing document.
        // Using the spread of req.body means the frontend can send the full
        // `char` state object directly (as CharacterSheet.jsx does).
        const { userId, _id, __v, createdAt, ...allowedUpdates } = req.body;
        ...
        res.status(500).json({ success: false, message: "Chyba pri ukladaní postavy", error: error.message });
    }
};
...
export const deleteCharacter = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Postava bola vymazaná" });

    } catch (error) {
        console.error("DELETE CHARACTER ERROR:", error);
        res.status(500).json({ success: false, message: "Chyba pri mazaní postavy", error: error.message });
    }
};